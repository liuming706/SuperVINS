...
class Extractor_DPL {
 public:
  ...
  void initialize(const std::string &extractorPath, int extractor_type_);
  ...
  // keypoints_result; std::pair<std::vector<cv::Point2f>,
  // std::vector<cv::Point2f>> descriptors_result;

  cv::Mat pre_process(const cv::Mat &Image, float &scale);
  std::pair<std::vector<cv::Point2f>, float *> post_process(
      std::vector<Ort::Value> tensor);
  std::pair<std::vector<cv::Point2f>, float *> extract_featurepoints(
      const cv::Mat &image);
...
class Matcher_DPL {
 public:
  ...
  void initialize(const std::string &matcherPath, int extractor_type_,
                  float matchThresh_);
  ...
  std::vector<cv::Point2f> pre_process(std::vector<cv::Point2f> kpts, int h,
                                       int w);
