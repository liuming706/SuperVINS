...
void FeatureTrackerDPL::initializeExtractorMatcher(
    int extractor_type_, string &extractor_weight_path,
    string &matcher_weight_path, float matcher_threshold = 0.5) {
  ...
  FeatureMatcherDPL->initialize(matcher_weight_path, extractor_type_,
                                matcher_threshold);
...
cv::Mat FeatureTrackerDPL::Extractor_PreProcess(const cv::Mat &Image,
                                                float &scale) {
  float temp_scale = scale;
  cv::Mat tempImage = Image.clone();
  // std::cout << "[INFO] Image info :  width : " << Image.cols << " height :  "
  // << Image.rows << std::endl;
  ...
  // std::endl; resultImage = RGB2Grayscale(resultImage);
  //}
...
void FeatureTrackerDPL::goodFeaturesToTrack_dpl(
    cv::Mat img, vector<cv::Point2f> &pts,
    vector<pair<cv::Point2f, vector<float>>> &dplpts_descriptors, int max_num,
    double extractor_threshold, int radius, cv::Mat &mask) {
  cv::Mat im = img.clone();
  cv::Mat im_preprocessed =
      Extractor_PreProcess(im, FeatureExtractorDPL->scale);
  ...
  for (int i = 0; i < n; i++) {
    ...
    cv::Point2f pt =
        cv::Point2f((dplpt.x + 0.5) / FeatureExtractorDPL->scale - 0.5,
...
void FeatureTrackerDPL::reduceVector(vector<cv::Point2f> &v,
                                     vector<uchar> status) {
  int j = 0;
...
void FeatureTrackerDPL::match_features_dpl(
    ...
    vector<pair<int, int>> &result_matches, double &ransacReprojThreshold) {
  // 定义特征点与描述子大小
  ...
  vector<cv::Point2f> prev_dplpts_normalized = FeatureMatcherDPL->pre_process(
      prev_dplpts, prev_img_.rows, prev_img_.cols);
  ...
  tem_matches = FeatureMatcherDPL->match_featurepoints(
      prev_dplpts_normalized, cur_dplpts_normalized, prev_descriptors,
      cur_descriptors);
  ...
  // Suppose there are two keypoint vectors named keypoints1 and keypoints2,
  // corresponding to two images respectively.
  ...
  if (points1.size() < 8 || points2.size() < 8) {
    std::cout << "points size less than 8, return empty matches" << std::endl;
    return;
  }
  cv::Mat fundamentalMatrix =
      cv::findFundamentalMat(points1, points2, cv::FM_RANSAC,
                             ransacReprojThreshold, 0.99, inliersMask);
...
void FeatureTrackerDPL::match_with_predictions_dpl(
    cv::Mat prev_img_, cv::Mat cur_img_,
    vector<pair<cv::Point2f, vector<float>>> &prev_dplpts_descriptors_,
    vector<pair<cv::Point2f, vector<float>>> &cur_dplpts_descriptors_,
    vector<cv::Point2f> &predict_pts_, vector<cv::Point2f> &cur_pts_,
    vector<pair<int, int>> &result_matches, double &ransacReprojThreshold) {
  int n_pre = prev_dplpts_descriptors_.size();
  int n_cur = cur_dplpts_descriptors_.size();
  vector<cv::Point2f> prev_dplpts, cur_dplpts;
  ...
  vector<pair<int, int>> matches = FeatureMatcherDPL->match_featurepoints(
      prev_dplpts_normalized, cur_dplpts_normalized, prev_descriptors,
      cur_descriptors);
  // RANSAC 参数
  ...
  cv::Mat fundamentalMatrix = cv::findFundamentalMat(
      prev_dplpts_normalized, cur_dplpts_normalized, cv::FM_RANSAC,
      ransacReprojThreshold, 0.99, inliersMask);
...
/// @brief sets the feature extraction mask and sorts according to the number of
/// tracking
void FeatureTrackerDPL::setMask() {
  ...
  // Store the feature points of the current frame according to (number of
  ...
  for (auto &it : cnt_pts_id) {
    if (mask.at<uchar>(it.second.first) == 255) {
      ...
      // If the pixel position of the feature point in the mask is 255, set a
      // "prohibited extraction area" with a radius of min dist around the mask,
      // and set the pixel to 0
      cv::circle(mask, it.second.first, MIN_DIST, 0, -1);
    }
  }
}

cv::Mat FeatureTrackerDPL::setMask_dpl(vector<cv::Point2f> &matched_points,
                                       int radius = MIN_DIST) {
  mask = cv::Mat(row, col, CV_8UC1, cv::Scalar(255));
...
FeatureTrackerDPL::trackImage_dpl(double _cur_time, const cv::Mat &_img,
                                  const cv::Mat &_img1) {
  TicToc t_r;
  // 当前时间
  // current time
  ...
  // 打开文件进行保存（以追加模式打开）
  std::ofstream outFile("time_consumption/feature_extraction_matching.txt",
                        std::ios::app);
  if (outFile.is_open()) {
  ...
  cout << cur_pts.size() << " points extracted in current frame originally"
       << endl;

  // 上一帧特征点不能为空，否则没法追踪
  // The feature points of the previous frame cannot be empty, otherwise they
  // cannot be tracked.\
  ...
  if (prev_pts.size() > 0) {
    ...
    if (hasPrediction) {
      ...
      match_with_predictions_dpl(prev_img, cur_img, prev_dplpts_descriptors,
                                 cur_dplpts_descriptors, predict_pts, cur_pts,
                                 matches, ransacReprojThreshold);
      ...
      // ROS_INFO_STREAM("cv::countNonZero(cur_img) = " <<
      // cv::countNonZero(cur_img));
      ...
      // 使用深度学习进行特征匹配
      // use deep-learning extractor to match feature points
      match_features_dpl(prev_img, cur_img, prev_dplpts_descriptors,
                         cur_dplpts_descriptors, matches,
    ...
    for (int i = 0; i < n_matches; i++) {
      ...
      temp_prev_dplpts_descriptors.push_back(
          prev_dplpts_descriptors[match.first]);
      temp_cur_pts.push_back(cur_pts[match.second]);
      temp_cur_dplpts_descriptors.push_back(
          cur_dplpts_descriptors[match.second]);
  ...
  // to avoid point gathering, create a mask depending on matched points in the
  // current image
  cv::Mat mask_for_dpl = setMask_dpl(temp_cur_pts, MIN_DIST);

  // add new feature points but filter the ones which are close to matched
  // points via the mask
  for (int i = 0; i < cur_pts.size(); i++) {
    if (!cur_matched_indices.count(i)) {
      if (mask_for_dpl.at<uchar>(cur_pts[i]) ==
          255)  // TO DO: annote this line can alleviate the problem of system
                // crashing
      {
  ...
  if (!_img1.empty() && stereo_cam) {
    ...
    if (!cur_pts.empty()) {
      extract_features_dpl(rightImg, cur_right_pts,
                           cur_dplpts_right_descriptors);
      vector<pair<int, int>> right_matches;

      // double ransac_thresh = 0.5;
      match_features_dpl(cur_img, rightImg, cur_dplpts_descriptors,
                         cur_dplpts_right_descriptors, right_matches,
                         ransacReprojThreshold);
      ...
      cur_un_right_pts = undistortedPts(
          cur_right_pts, m_camera[1]);  // 去畸变加反投影至归一化平面
      right_pts_velocity = ptsVelocity(
          ids_right, cur_un_right_pts, cur_un_right_pts_map,
          prev_un_right_pts_map);  // 计算右目特征点在相机归一化平面上的速度
    }
  ...
  prev_un_pts =
      cur_un_pts;  // 当前帧去畸变反投影特征点赋给上一帧去畸变反投影特征点
  prev_un_pts_map =
      cur_un_pts_map;  // 上一帧去畸变反投影特征点的map=当前帧去畸变反投影特征点map：（特征点id,特征点坐标）
  ...
  map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>>
      featureFrame;  // 要准备返回特征帧了：key值为featureid，value值为map：key值为相机id，value值为7x1向量，分别存放[归一化平面x,归一化平面y,1,像素平面x,像素平面y,归一化平面速度x,归一化平面速度y]
  for (size_t i = 0; i < ids.size(); i++) {
    ...
    xyz_uv_velocity << x, y, z, p_u, p_v, velocity_x,
        velocity_y;  // 把数据存入向量
    featureFrame[feature_id].emplace_back(camera_id,
                                          xyz_uv_velocity);  // 形成当前帧的特征
  }
  // 右目是一样的流程
  if (!_img1.empty() && stereo_cam) {
...
void FeatureTrackerDPL::readIntrinsicParameter(
...
/// @param cam
/// @return
vector<cv::Point2f> FeatureTrackerDPL::undistortedPts(
  ...
  for (unsigned int i = 0; i < pts.size(); i++) {
    ...
    Eigen::Vector3d b;
    cam->liftProjective(
        a, b);  // 针孔相机模型下，返回的b就是去畸变的归一化相机平面坐标[x,y,1]
    un_pts.push_back(cv::Point2f(
        b.x() / b.z(),
        b.y() /
            b.z()));  // 针孔相机模型下，b.z()是1.0，所以横纵坐标为归一化平面坐标
  }
  return un_pts;
}
...
/// @return
vector<cv::Point2f> FeatureTrackerDPL::ptsVelocity(
    ...
    for (unsigned int i = 0; i < pts.size(); i++) {
      ...
      if (it != prev_id_pts.end())  // 能找到和上一帧对应的特征点，则计算速度
      {
        ...
        pts_velocity.push_back(
            cv::Point2f(0, 0));  // 不能找到和上一帧对应的特征点，则速度为0
    }
...
void FeatureTrackerDPL::setPrediction(map<int, Eigen::Vector3d> &predictPts) {
  ...
  map<int, Eigen::Vector3d>::iterator
      itPredict;  // 设置一个存放路标点的map的迭代器
  // 遍历待追踪的特征点的id
  for (size_t i = 0; i < ids.size(); i++) {
    ...
    if (itPredict != predictPts.end())  // 如果没找到
    {
      ...
      m_camera[0]->spaceToPlane(
          itPredict->second, tmp_uv);  // 将预测的相机系下的空间点投影至图像像素
      predict_pts.push_back(
          cv::Point2f(tmp_uv.x(), tmp_uv.y()));  // 存入预测特征点像素
      predict_pts_debug.push_back(cv::Point2f(tmp_uv.x(), tmp_uv.y()));
    } else
      predict_pts.push_back(
          prev_pts
              [i]);  // 若是没有预测特征点，则利用历史特征点的位置（这其实就是不通过初始光流时的结果）
  }
}
...
cv::Mat FeatureTrackerDPL::getTrackImage() { return imTrack; }